class ReservationSystem {
    ...
    async handleDateSelection() {
        ...
        try {
            ...
            if (!data.available) {
                ...
                this.showDateInfo(`${this.availableSlots.length} Zeitslots verfügbar`);
                const nextBtn = document.getElementById('btn-next-date');
    ...
    async submitReservation(e) {
        ...
        this.reservationData.guests = parseInt(formData.get('guests'), 10);
    ...
    showConfirmation(result) {
        ...
        if (codeEl) codeEl.textContent = result.confirmationCode;
        if (nameEl) nameEl.textContent = this.reservationData.name;
        if (dateEl) dateEl.textContent = this.formatDateDisplay(this.reservationData.date);
        if (timeEl) timeEl.textContent = `${this.reservationData.time} Uhr`;
        if (guestsEl) {
            const guests = this.reservationData.guests;
            guestsEl.textContent = `${guests} ${guests === 1 ? 'Person' : 'Personen'}`;
        }
        if (emailEl) emailEl.textContent = this.reservationData.email;
