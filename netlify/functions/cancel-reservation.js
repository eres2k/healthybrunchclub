...
const { loadReservationsForDate, saveReservationsForDate, getReservationDateTime } = require('./utils/reservation-utils');
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const reservationDateTime = getReservationDateTime(foundReservation.date, foundReservation.time);
    const now = new Date();
    const hoursUntilReservation = reservationDateTime
      ? (reservationDateTime - now) / (1000 * 60 * 60)
      : Infinity;
