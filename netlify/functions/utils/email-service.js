...
async function generateEmailHTML(reservation, isAdmin = false) {
  const qrCode = reservation.status === 'confirmed'
  ...
  const introText = isAdmin
    ? (reservation.status === 'cancelled'
        ...
        : reservation.status === 'waitlist'
          ? 'Eine neue Wartelisten-Anfrage wurde eingetragen:'
          : 'Eine neue Reservierung wurde vorgenommen:')
    : (reservation.status === 'cancelled'
        ? `Liebe/r ${reservation.name}, Ihre Reservierung wurde erfolgreich storniert.`
        : reservation.status === 'waitlist'
          ? `Liebe/r ${reservation.name}, Sie stehen nun auf unserer Warteliste. Wir informieren Sie, sobald ein Platz frei wird.`
    ...
    <div style="padding:40px 30px;">
      ...
      <div style="text-align:center; margin-top:40px; padding-top:30px; border-top:1px solid #e0e0e0;">
        <p style="color:#666; font-size:14px; margin:0 0 10px;">
          ${!isAdmin 
            ? (reservation.status === 'cancelled' ? 'Wir hoffen, Sie bald wieder bei uns begrüßen zu dürfen.' : 'Wir freuen uns auf Ihren Besuch!')
            : (reservation.status === 'cancelled' ? 'Bitte im System vermerken' : 'Bitte im System bestätigen')}
        </p>
...
async function sendReservationEmails(reservation) {
  ...
  const customerSubjects = {
    confirmed: '✅ Reservierungsbestätigung - Healthy Brunch Club Wien',
    waitlist: '🕐 Warteliste - Healthy Brunch Club Wien',
    cancelled: '❌ Stornierungsbestätigung - Healthy Brunch Club Wien'
  };

  const adminSubjects = {
    confirmed: '🔔 Neue Reservierung',
    waitlist: '🔔 Neue Reservierung (Warteliste)',
    cancelled: '⚠️ Reservierung storniert'
  };

  const normalizedStatus = ['confirmed', 'waitlist', 'cancelled'].includes(reservation.status)
    ? reservation.status
    : 'confirmed';
  ...
  const customerEmail = {
    ...
    subject: customerSubjects[normalizedStatus],
    html: customerHTML,
    attachments
  };
  ...
  const adminEmail = {
    ...
    from: process.env.SENDER_EMAIL || 'noreply@healthybrunchclub.at',
    subject: `${adminSubjects[normalizedStatus]} - ${formatDateAustrian(reservation.date)} ${reservation.time} - ${reservation.guests} Personen`,
    html: adminHTML,
