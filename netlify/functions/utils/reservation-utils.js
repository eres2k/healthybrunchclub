...
async function getAvailability({ date, guests }) {
  ...
  const slots = timeSlots.map((time) => {
    ...
    const capacity = cmsConfig.capacity ||
                    blockConfig?.capacity ||
                    settings.maxCapacity ||
                    MAX_CAPACITY_PER_SLOT;

    // Check if slot is blocked
    const isBlocked = cmsConfig.blocked || false;
    ...
    // Preserve configured capacity for UI display
    ...
    if (isBlocked) {
      ...
      slot.fits = false;
      slot.waitlist = false;
    } else if (blockReason && effectiveCapacity === 0) {
      slot.blockReason = blockReason;
      slot.blockedReason = blockReason;
    }
